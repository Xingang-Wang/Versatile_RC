function min_rmse= func_train_repeat_Kur_para(hyperpara_set,repeat_num,train_data,train_output_data,testdata,test_output_data,train_length,train_num)
% repeat func_train
%rng(2);
...
for repeat_i = 1:repeat_num
   ...
   % rng_num=repeat_i*20000 + (now*1000-floor(now*1000))*100000;
    rng_num=randi(500);
    rng(rng_num)
    [rmse,rmse_dynamic] = func_train_Kur_para(hyperpara_set,rng_num,train_data,train_output_data,testdata,test_output_data,train_length,train_num);
    if rmse<=min_rmse
        min_rmse=rmse;
        min_rng=rng_num;
