function [rmse,rmse_dynamic]  = func_train_Kur_para(degree,hyperpara_set,rng_num,train_data,train_output_data,testdata,test_output_data,train_length,train_num)
%%
resSize =500;
rng(rng_num);
drive_num=50;
...
for t = 1:test_Len-1 
    ...
    Y1(:,t+1) = y;
     if t<drive_num
     %u(1:2*max_d)=Testdata(1:2*max_d,1+t);
     u(2*(max_d+1)-1)=Testdata(2*(max_d+1)-1,1+t);
     u(2*(max_d+1))=Testdata(2*(max_d+1),1+t);
    else
    u(2*(max_d+1)-1:2*(max_d+1)) = y;  
     end  
     u(1:2*max_d)=Testdata(1:2*max_d,1+t);
    u(2*(max_d+1)+1:2*(max_d+1)+2)=Testdata(2*(max_d+1)+1:2*(max_d+1)+2,1+t);
end
rmse_dynamic=rmse_dynamic+mean(abs(Y1(1,drive_num+1:testLen)-...
    Testdata(9,drive_num+1:testLen)))+...
    mean(abs(Y1(2,drive_num+1:testLen)-...
    Testdata(10,drive_num+1:testLen)));
