function [rmse,rmse_dynamic]  = func_train_chua_coupled(hyperpara_set,rng_num,train_data,train_output_data,testdata,test_output_data,train_length,train_num)
%
resSize =500;
rng(rng_num);
drive_num=50;
...
for i=1:train_num
    indata=train_data{i};
    [X1,W,reg]=func_get_X(resSize,hyperpara_set,rng_num,indata,Win,train_length);
    X=[X,X1];
end
X=X(:,1:end-1);
%% 
...
for i=1:testnum
    Testdata=testdata{i};
    u=Testdata(1:max_d+4,1);
    Y1(:,1)=u(max_d+1:max_d+3);
...
%     u(1:3) = y;   
%      end  
%     u(4:end)=Testdata(4:3*(d+1)+2,1+t);
     if t<drive_num
     u(max_d+1)=Testdata(max_d+1,1+t);
     u(max_d+2)=Testdata(max_d+2,1+t);
     u(max_d+3)=Testdata(max_d+3,1+t);
    else
    u(max_d+1:max_d+3) = y;   
     end  
       u(1:max_d)=Testdata(1:max_d,1+t);
       
       u(max_d+4)=Testdata(max_d+4,1+t);
    end
end
rmse_dynamic=rmse_dynamic+mean(abs(Y1(1,drive_num+1:testLen)-...
    Testdata(max_d+1,drive_num+1:testLen)))+...
    mean(abs(Y1(2,drive_num+1:testLen)-...
    Testdata(max_d+2,drive_num+1:testLen)))+...
    mean(abs(Y1(3,drive_num+1:testLen)-...
    Testdata(max_d+3,drive_num+1:testLen)));
end
